...
function checkAuth() {
    ...
    console.log('🔍 checkAuth - user:', user, 'type:', userType);
    
    if (!user || !userType) {
        const currentPath = window.location.pathname;
        console.log('❌ No session found, current path:', currentPath);
        
        // Check if we're not on the login page
        const isLoginPage = currentPath.includes('index.html') || 
                           currentPath === '/' || 
                           currentPath.endsWith('/amisty-pos/') ||
                           currentPath.endsWith('/amisty-pos');
    ...
    console.log('✅ Auth OK - User type:', App.userType);
    return true;
}
